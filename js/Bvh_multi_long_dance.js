...
BVH.Reader.prototype = {
    ...
    parseData:function(index, data){
		...
		debugTell(this.index, "BVH frame:"+this.numFrames+" s/f:"+this.secsPerFrame + " channels:"+this.channels.length + " node:"+ this.nodes.length);
		this.getNodeList();
    ...
    getNodeList:function () {
		...
		if (this.index == 0) {
			if (out12) out12.innerHTML = s;
		}
		else if (this.index == 1) {
			if (out22) out22.innerHTML = s;
		}
		else {
			if (out32) out32.innerHTML = s;
		}
    },
    ...
    addSkeleton:function ( n ) {
		...
		if (this.index == 0) {
			scene1.add( this.skeleton );
		}
		else if (this.index == 1) {
			scene2.add( this.skeleton );
		}
		else {
			scene3.add( this.skeleton );
		}
    ...
    updateSkeleton:function () {
    	...
    	for(var i=0; i<n; i++){
    		...
    		if ( node.name !== 'Site' ){
				...
				// if (this.frame > 0 && this.frame <= this.n_past) {
				// 	bone.material.color.set(0xf3b22e);
    ...
    clearNode:function(){
		...
		if (this.index == 0) {
			if (out12) out12.innerHTML = "";
		}
		else if (this.index == 1) {
			if (out22) out22.innerHTML = "";
		}
		else {
			if (out32) out32.innerHTML = "";
		}
    	...
    	if(this.nodes){
			...
			if(this.bones.length > 0){
				...
				if (this.index == 0) {
					scene1.remove( this.skeleton );
				}
				else if (this.index == 1) {
					scene2.remove( this.skeleton );
				}
				else {
					scene3.remove( this.skeleton );
				}
		   }
		}
    },
