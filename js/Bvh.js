...
BVH.Reader.prototype = {
    ...
    updateSkeleton:function () {
		if (this.frame > 0 && this.frame <= this.n_past) {
			debugTell("BVH frame:"+this.numFrames+" s/f:"+this.secsPerFrame + " channels:"+this.channels.length + " node:"+ this.nodes.length+'<br/>(motion A) '+this.frame);
		}
		else if (this.frame > this.n_past && this.frame <= this.n_past+this.n_trans) {
			debugTell("BVH frame:"+this.numFrames+" s/f:"+this.secsPerFrame + " channels:"+this.channels.length + " node:"+ this.nodes.length+'<br/>(Transition) '+this.frame);
		}
		else if (this.frame > this.n_past+this.n_trans && this.frame <= this.n_past+this.n_trans+this.n_future) {
			debugTell("BVH frame:"+this.numFrames+" s/f:"+this.secsPerFrame + " channels:"+this.channels.length + " node:"+ this.nodes.length+'<br/>(motion B) '+this.frame);
		}
		else {
			debugTell("BVH frame:"+this.numFrames+" s/f:"+this.secsPerFrame + " channels:"+this.channels.length + " node:"+ this.nodes.length+'<br/>(Invalid) '+this.frame);
		}
    ...
    update:function(){
    	if ( this.play ) { 
			this.frame = ((((Date.now() - this.startTime) / this.secsPerFrame / 1000) )*this.speed)| 1;
			if(this.oldFrame!==1)this.frame += this.oldFrame;
			if(this.frame > this.numFrames ){this.frame = 1;this.oldFrame=1; this.startTime =Date.now() }
    ...
    next:function(){
    	this.play = false;
    	this.frame ++;
    	if(this.frame > this.numFrames )this.frame = 1;
